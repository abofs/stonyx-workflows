...
| `test/helpers/checkout-credentials.js` | The #35 guard, off raw text -- `match`, `split`, `trim`, and nothing imported from `workflow-yaml.js`. Fail-closed at every judgement: a checkout it cannot bound to a step **throws**, a duplicate `token:` **throws**, an unrecognised token spelling counts as **privileged**, and an unrecognised `persist-credentials` value counts as **not disabled**. `rawCheckoutCount` counts the population off raw text with code sharing nothing with the reader, so a checkout the reader cannot see reds on the count |
